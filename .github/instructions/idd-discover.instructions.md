...
3. Apply A3's readiness bullets to the target — no configured
   ...
   `web-toybox-blocked-by` marker, both resolved the
...
NOT contain a `web-toybox-roadmap-id` marker (not itself
a roadmap) or a `web-toybox-blocked-by` marker, AND
...
- **Roadmap identity** (`web-toybox-roadmap-id`): placed in
...
- **Sequential dependency** (`web-toybox-blocked-by`): placed
