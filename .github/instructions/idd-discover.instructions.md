...
2. If the target issue carries the configured roadmap label or an
   `web-toybox-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `web-toybox-blocked-by` markers) and its
...
NOT contain an `web-toybox-roadmap-id` marker (not itself
a roadmap) or an `web-toybox-blocked-by` marker, AND
...
`web-toybox-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`web-toybox-roadmap-id` marker. Retro-label a legacy
...
`<!-- web-toybox-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `web-toybox-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `web-toybox-roadmap-id` or
  `web-toybox-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `web-toybox-roadmap-id` marker when checking
  `web-toybox-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- web-toybox-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- web-toybox-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- web-toybox-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- web-toybox-effort: S|M|L -->` footer
...
- **Roadmap identity** (`web-toybox-roadmap-id`): in the
...
- **Sequential dependency** (`web-toybox-blocked-by`): in an
  issue body — this issue **cannot start until** the roadmap with the
  matching `roadmap-id` is closed.

**Do not use `web-toybox-blocked-by` to group sub-tasks under
