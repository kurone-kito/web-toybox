...
| -------------------------------- | --------------------------------------------------------- | ---------------------------------- |
| `web-toybox`                  | Repository short name used in worktree examples           | `my-app`                           |
| `web-toybox`      | Hidden issue-body marker prefix                           | `my-app`                           |
...
| `corepack enable && pnpm install --prefer-frozen-lockfile`       | Dependency install command, or `true` when unnecessary    | `npm install`                      |
...
- no dependency install step →
  `corepack enable && pnpm install --prefer-frozen-lockfile = true`
- no relevant auto-fix command →
  `pnpm run lint:fix && pnpm run lint = true`

Keep `corepack enable && pnpm install --prefer-frozen-lockfile` safe to rerun across retries, takeovers,
and recreated worktrees.

## Marker prefix notes

`web-toybox` appears in two hidden issue-body markers:

- roadmap identity marker:
  `<!-- web-toybox-roadmap-id: {unique-id} -->`
- blocked-by marker:
  `<!-- web-toybox-blocked-by: {roadmap-id} -->`
